...
function templateOpt(pdi,mi){const perm=S.permMeals[pdi+"_"+mi];if(perm)return perm;return RICETTE[pdi].meals[mi].o[0];}
function plannedTemplateOfDay(di){let k=0,p=0,c=0,f=0,fib=0,z=0;RICETTE[di].meals.forEach((m,mi)=>{const o=templateOpt(di,mi);k+=o.k;p+=o.p;c+=(o.c||0);f+=(o.f||0);fib+=(o.fib||0);z+=(o.z||0);});return{k,p,c,f,fib,z};}
function mealCardStatic(pdi,mi){const m=RICETTE[pdi].meals[mi];const o=templateOpt(pdi,mi);
...
window.setPermOpt=(d,m,oi)=>{const o=RICETTE[d].meals[m].o[oi];
...
window.editMealPermGo=async(d,m)=>{
  ...
  S.permMeals[d+"_"+m]={d:txt,k,p,c,f};ricetteCambiate();save();render("ricette");};
window.altMealPerm=async(d,m)=>{if(!aiOn())return aiFail(new Error("nokey"));const o=templateOpt(d,m);
  ...
  try{const t=await aiAsk('Genera UN piatto completamente diverso da "'+o.d+'" ma con circa '+o.k+' kcal e '+o.p+' g di proteine. '+
    (visti.length?'Alternative GIÀ PROPOSTE in passato per questo pasto — NON riproporle né in variante simile: '+visti.join("; ")+'. ':'')+vincoliStr()+' Ingredienti comuni in Italia con grammature. Rispondi SOLO JSON: {"piatto":"descrizione con grammature","kcal":numero,"prot":numero,"carb":numero,"gras":numero}');
...
function renderRicette(){const el=document.getElementById("pg-ricette");const ti=wd(new Date());
  ...
  /* ricetteVuote() esiste già nel modulo dello stato: si riusa, non si
  ...
  ${(function(){const d=planWeightDrift();
    ...
    /* nelle unità della persona (28/08, seconda passata): questo
  ...
  /* Il piano di partenza SI DICHIARA (riscontro del founder, 25/08
     sera: «quello generato è il mio piano, non un piano generato
     dall'AI» — l'app gliel'aveva dato come ripiego senza dirglielo
...
function puntoSaluto(){
  ...
  return s+(n?", "+esc(n):"");}
/* Il primo giorno d'uso si riconosce dai DATI, non da una data
   scritta da qualche parte: se non c'è niente di ieri o prima —
   né pesate, né settimana archiviata, né pasti segnati — allora
   ieri l'app non c'era, e di ieri non si parla. */
function primoGiornoDiUso(){
  try{
    const oggi=iso(new Date());
    if((S.profile.weights||[]).some(w=>w&&w.d&&w.d<oggi))return false;
    if(((S.archWeeks||S.hist)||[]).length)return false;
    if((S.week.days||[]).some(d=>(d&&d.date&&d.date<oggi)&&((d.meals&&Object.keys(d.meals).length)||(d.extras||[]).length)))return false;
    return true;
...
function puntoTesta(di){
  ...
  }else if(!primoGiornoDiUso()){
    /* «Ieri non hai segnato nulla» detto a chi ha fatto l'onboarding
       OGGI è un rimprovero per un giorno in cui l'app non esisteva
       (riscontro del founder, 25/08 sera). Il promemoria vale solo
       da domani in poi. */
    righe.push(tr("Ieri non hai segnato nulla: se vuoi puoi ancora farlo dallo storico."));}
...
function pesoRiga(){
  ...
  /* stessa cosa qui: questa riga è il titolo del riquadro peso sul
     Punto, la prima cosa che si vede aprendo l'app */
  const dTxt=(typeof pesoTxt==="function")?pesoTxt(Math.abs(d),1):num(d)+" kg";
  const testa=(d===0?tr("Stabile")+" ":(d<0?"−"+dTxt+" ":"+"+dTxt+" "));
...
function renderPunto(){const el=document.getElementById("pg-punto");const di=viewIdx();
  ...
  if(!S.ui.vacanza){
    ...
    <div class="glist">
      ...
      :`<div class="grow">
        ...
        <div class="gc"><button class="glink" onclick="periodoApri()">${esc(tr("Aprine uno"))} ›</button></div>
      </div>`)}
  ...
  h+=(function(){
      ...
      /* kcalNum: in Modalità Zen questi due frammenti — che sono numeri —
         spariscono; «Resta un pasto» resta, perché dice cosa fare, non
         quanto pesa (v15.5.0) */
      if(burn)sotto.push('<span class="kcalNum">'+(burn>0?"+":"")+burn+" kcal di sport</span>");
      if(q!=null)sotto.push('<span class="kcalNum">qualità '+q+"%</span>");
      return `<div class="today">
        <div class="tanswer"><span class="${ton}">${cap}</span>${(previsti&&dopo.length)?`<span class="kcalNum"> ${tr("Restano")} <b>${resta}</b> kcal.</span>`:""}</div>
        <div class="tsub">${sotto.join(" · ")||(previsti?"Segna il primo pasto per iniziare":"Chiedi le ricette o aggiungi quello che mangi con + Extra")}</div>
        <div class="tmeter">
          <div class="trow"><span>Mangiate</span><b class="kcalNum">${eat.k} / ${plan}</b></div>
        ...
        ${prox?`<div class="tnext">
          ...
          <div class="tcta">
            <button class="btn small" onclick="tgl(${prox.pdi},${prox.mi})">${esc(tr("L'ho mangiato"))}</button>
  ...
  if(densMin("full")){
  h+=`<div class="card"><h2>${tr("Sonno e umore")}</h2>
  ...
  /* ── TESTO SOPRA, COMANDI SOTTO (founder, 26/08 sera) ────────────
     Questi due avvisi stavano su UNA riga flessibile: il testo aveva
     flex:1 con base zero, i tre bottoni non si stringono, e su un
     telefono il testo finiva in una colonna da ottanta pixel — una
     parola per riga, «laterale e non in alto». Un avviso lungo non
     sta DI FIANCO ai suoi comandi: sta sopra. */
  if(needWeekBanner())avvisi.push(`<div class="card nota info">
    <div style="font-size:13px"><b>${tr("Nuova settimana.")}</b> ${tr("La scorsa è stata archiviata da sola: se vuoi darle un'occhiata o correggere qualcosa, la trovi in Storico →  Modifica.")}</div>
    <div class="mtools">
  ...
  if(!aiOn()&&!S.ui.dismissAI)avvisi.push(`<div class="card nota">
    ...
    <div class="mtools">
      ...
      <button class="ibtn" onclick="S.ui.dismissAI=1;save();render('oggi')" title="${esc(tr("Nascondi"))}">${ic("x",15)}</button></div></div>`);
...
function renderOggi(){const el=document.getElementById("pg-oggi");const di=viewIdx();
  const eat=eatenOfDay(di),burn=burnedOfDay(di),t=tdeeOfDay(di),def=t+burn-eat.k,target=t+burn;
  let h="";
  /* La fascia del modo banco sta in cima, prima di qualunque altra
     cosa: un avviso che si trova scorrendo non è un avviso. */
  ...
  h+=`<div class="card"><h2>${tr("I pasti")}</h2>${hint2(tr("Tocca il cerchio: ○ da fare · ✓ mangiato · ✗ saltato."),
   tr("⏳ compare quando l'AI sta elaborando. I comandi sotto ogni pasto: matita per modificare, dado per un'alternativa, frecce per sostituire un ingrediente, macchina fotografica e galleria per la stima dalla foto, barcode per i prodotti confezionati."),null,tr("tutti i comandi"))}</div>`;
  ...
  h+=`<div class="gaugecard"><h2>${tr("Modalità vacanza")}</h2>
    ...
    h+=`<div class="gaugecard">${arcoGiornoHTML(di)}<div class="hint" style="margin:0 0 4px">${tr("Bilancio di")} ${giorno(RICETTE[di].day)} ${isToday()?"":("("+VIEW.toLocaleDateString(dataLoc())+")")}${isHard(_vd)?` · <span style="color:var(--zafft)"> ${esc((S.dayEvents||{})[_vd]||"giornata difficile")} (esclusa dalle medie)</span>`:""}</div>
    ${(function(){
      ...
      const q=dayQuality(di);
      /* ═══ SOLO BARRE (decisione del founder, 25/08 sera) ═══════
         «Sul bilancio di oggi le cose si ripetono. I quadrati —
         deficit, calorie, proteine eccetera — puoi toglierli tutti:
         usiamo solo le barrette. E fai diventare una barra anche la
         qualità del cibo.»
         Kcal e macro vivono già nell'arco e nelle barre in alto: i
         riquadri li RIPETEVANO (ed erano quadrati, non barre). Qui
         restano le tre misure che in alto non ci sono — fibre,
         zuccheri (dove l'obiettivo è un TETTO: oltre, la barra
         diventa rossa), qualità — tutte nella stessa forma. Con la
         densità piena fibre e zuccheri c'erano già; ora ci sono per
         tutti: erano il riscontro «non ci sono le barre di zuccheri
         e fibre in basso». */
      return `<div class="mtx soloBarre">
      ${box(eat.fib," g",tr("fibre"),dayTargetFib(),"#00AFA3")}
      ${box(eat.z," g",tr("zuccheri"),dayTargetZ(),"#FF7F50",true)}
      ${(q!=null)?box(q,"%",tr("qualità del cibo"),100,"var(--bosco)")
                 :`<div><div class="l">${tr("qualità del cibo")}</div><div class="s">${aiOn()?tr("spunta i pasti per saperlo"):tr("serve la chiave AI")}</div></div>`}
    </div>`;})()}
...
function renderScanQ(){
  ...
  let inner=`<div class="mcard"><h2 style="color:var(--bosco);font-size:16px">Barcode — ${SCANQ.pdi!=null?"per il pasto "+esc(RICETTE[SCANQ.pdi].meals[SCANQ.mi].n):(SCANQ.ei!=null?"per questo extra":"come extra")}</h2>
