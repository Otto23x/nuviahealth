...
window.ruotaDopoSalvataggio=()=>{
  ...
  try{
    ...
    if(nuovi.length){
      ...
      try{toastFatto(nuovi[0].n+" · "+nuovi[0].t);}catch(e){}
...
window.renderRuota=()=>{
  ...
  if(ruotaComplete())h+=ruotaZenHTML();
  h+=ruotaAlfabetoHTML();
...
/* ── LA MODALITÀ ZEN ──────────────────────────────────────────────
   Il premio non aggiunge: toglie. Chi ha acceso tutti i segni può
   spegnere i numeri e restare con i colori. Si attiva a mano — un
   premio che scatta da solo è una funzione che cambia l'app sotto
   le mani di chi la stava usando. E si spegne quando si vuole. */
function ruotaZenHTML(){
  const on=!!(S.ui&&S.ui.zen);
  return `<div class="card zenCard">
    <h2>${esc(tr("Modalità Zen"))}</h2>
    <div class="hint">${esc(tr("Ventiquattro segni accesi: i numeri hanno finito il loro lavoro. Se vuoi, l'app li mette via e resta con i colori. Puoi riaccenderli quando vuoi."))}</div>
    <div class="mtools"><button class="btn${on?" ghost":""}" onclick="zenAttiva(${on?"false":"true"})">${
      on?esc(tr("Rivoglio i numeri")):esc(tr("Metti via i numeri"))}</button></div></div>`;}

window.zenAttiva=(si)=>{
  S.ui=S.ui||{};S.ui.zen=!!si;
  try{document.body.classList.toggle("zen",!!si);}catch(e){}
  save();
  toast(si?tr("Modalità Zen accesa."):tr("Numeri di nuovo visibili."));
  try{render(cur);}catch(e){}};

/* All'avvio la classe segue lo stato salvato: senza questo la Zen
   sparirebbe a ogni riapertura. */
try{setTimeout(()=>{try{
