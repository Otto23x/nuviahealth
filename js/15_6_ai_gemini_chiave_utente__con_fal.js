...
/* ═══ TAGLIO PENSIERO · LA TABELLA, SCRITTA UNA VOLTA ═════════════
   ...
   ── LE FOTO SONO TORNATE A «low» COL RESTO (27/08) ─────────────
   Il 25/08 erano scese a «minimal» per misurare quanto costasse quel
   livello. La misura è finita: «minimal» è fuori, e le foto seguono
   la regola generale come tutto il resto. Se il riconoscimento dei
   piatti dovesse peggiorare, il posto dove rimediare resta questa
   tabella — non un `if` sparso da qualche parte.                  */
/* ── TUTTO SU «low» (founder, 27/08) ──────────────────────────────
   «Il livello di ragionamento per il momento lo teniamo tutto su low.
   Escludiamo minimal, che è inutile e tutti i livelli sopra il medio,
   che non servono. Lo testo per un po' su low, se è sufficiente e
   qualitativo lo teniamo così, altrimenti passiamo a medium su tutto
   o diversifichiamo in base a quello che deve produrre.»

   Quindi la tabella oggi ha UN valore solo, e i livelli ammessi sono
   due: «low» e «medium». «minimal» non esiste più — è il livello che
   gemini-3.7-flash rifiuta in blocco e che sugli altri produceva
   risposte tagliate — e «high» nemmeno: nessuna delle nostre domande
   lo giustifica, e farebbe aspettare la persona per niente.
...
/* Solo i due livelli ammessi: chi sale, sale al massimo a «medium».
   Togliere «minimal» da qui non è cosmetico — è quello che impedisce a
   `pensieroAlza` di rimetterlo in circolo partendo da un modello che
   l'ha rifiutato. */
const PENSIERO_ORD=["low","medium"];
...
/* -- E AZZERA TUTTI E TRE I CONTATORI (founder, 27/08) ------------
   «Dopo la generazione del piano resta per molto tempo il messaggio
   dell'AI che sta lavorando per la lista della spesa, ma credo si sia
   bloccato.»
   Si era bloccato davvero, e questa rete non lo prendeva: azzerava
   solo AIBUSY. La spia pero' si accende anche con AILUNGO (le attese
   lunghe: piano, spesa, import) e con AIFILE, e quei due restavano
   contati. Togliere la classe non bastava: il primo ridisegno di
   qualunque pagina chiama aiSpiaSincro, che li rilegge e la
   riaccende. Cosi' la spia tornava da sola, per sempre, senza niente
   dietro. Adesso si azzera lo stato, non l'aspetto - e si chiudono
   anche gli scheletri rimasti aperti, che sono la stessa attesa
...
setInterval(()=>{
  ...
  AIBUSY=0;AILUNGO=0;AIFILE=0;
  try{skelChiudiTutti();}catch(_){}
  aiSpiaSincro();
      ...
      try{
        ...
        if(r.status===400){
          ...
          if(/thinking/i.test(det)){
            const cur=(cfg.thinkingConfig&&cfg.thinkingConfig.thinkingLevel)||"low";
...
/* ═══ LA PROVA DI CONTATTO, PRIMA DI CHIEDERE IL PIANO ════════════
   RICHIESTA DEL FOUNDER (27/08): «bisogna fare in modo, prima di
   passare il prompt al modello, che il sistema faccia una chiamata
   per vedere se risponde e solo dopo chiedere di generare il piano,
   altrimenti si rischia che il piano non arrivi mai alla fine.»

   Il punto non è risparmiare una chiamata: è QUANDO si scopre il
   guasto. Il piano è la richiesta più lunga che facciamo — minuti,
   con la persona che guarda una barra — e se la chiave è sbagliata,
   la quota è finita o la rete è giù, oggi lo si scopre alla fine di
   quei minuti. Questa è una domanda da due parole: risponde o non
   risponde, e si sa subito.

   Tre cose la rendono onesta:
   1. HA UN TEMPO SUO (25 secondi). Un piano può metterci minuti; una
      prova che ci mette minuti non è una prova, è l'attesa che
      volevamo evitare.
   2. NON DECIDE DUE VOLTE. Torna {ok:false, motivo} col motivo già
      tradotto da aiReason: chi chiama scrive quello, e non deve
      reinventare le diagnosi.
...
async function aiProva(){
  ...
  try{
    const risposta=await Promise.race([
      aiAsk("Rispondi con una sola parola: OK.","prova"),
    ...
    try{
      ...
      if(!r.ok){
        ...
        if(r.status===400&&/thinking/i.test(det)&&giro<1){
          const cur=(gemPensiero(m,"x",null,null).thinkingConfig||{}).thinkingLevel||"low";
