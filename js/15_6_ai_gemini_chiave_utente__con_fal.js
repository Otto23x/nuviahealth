...
function gemPensiero(modello,prompt,imgs,pilastro){
  return {thinkingConfig:{thinkingLevel:pensieroDi(pilastro)}};}
...
function toast(msg){const e=document.getElementById("toast");if(!e)return;
  ...
  const annullabile=!TOAST_SENZA_UNDO
    &&(typeof undoDisponibile==="function")&&undoDisponibile()
    &&(Date.now()-((typeof UNDO!=="undefined"&&UNDO.at)||0)<1500);
  const chiudi='<button class="tclose" onclick="toastVia()" aria-label="'+
    (typeof LANG!=="undefined"&&LANG==="en"?"Close":"Chiudi")+'">×</button>';
  if(annullabile){
    e.innerHTML=esc(testo)+' <button class="tundo" onclick="annullaUltima()">'+
      (typeof LANG!=="undefined"&&LANG==="en"?"Undo":"Annulla")+'</button>'+chiudi;
    e.classList.add("on");clearTimeout(toastT);
    toastT=setTimeout(()=>e.classList.remove("on"),pesante?15000:8000);
    return;}
  e.innerHTML=esc(testo)+chiudi;
  e.classList.add("on");clearTimeout(toastT);
  toastT=setTimeout(()=>e.classList.remove("on"),pesante?8000:2600);}
/* Un messaggio che CONSTATA, non che agisce: niente Annulla, qualunque
   cosa sia appena stata salvata. */
let TOAST_SENZA_UNDO=false;
