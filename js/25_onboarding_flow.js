...
function onb2Integ(sc){
  ...
  h+=`<div class="o2ops o2multi" data-multi="${esc(sc.k)}">`+sc.op.map(([v,t,d])=>{
    ...
    if(v==="nessuno")
      return `<div class="o2irowi o2irowi-piena"><button class="o2op o2opm${on?" scelta":""}" type="button" data-v="${esc(v)}"
    ...
    const tendina=`<select class="o2ifreq" ${on?"":"disabled"} aria-label="${esc(trh("Ogni quanto: {v1}",{v1:t}))}"
...
function onb2Fine(sc){
  ...
  return `${masc(pronto?"festeggia":"cucina",120)}
