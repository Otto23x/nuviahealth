...
function ONB2t(){return [
 ...
 {k:"attivita",sez:"vita",tipo:"scelta",
  q:tr("Come sono fatte le tue giornate?"),
  sub:tr("Conta la giornata intera, non solo l'allenamento: è da lì che esce quanto consumi."),
  op:[["fermo",tr("Ferma quasi tutto il giorno"),tr("Scrivania o divano, nessun allenamento")],
      ["poco",tr("Seduta, ma qualche camminata"),tr("Nessun allenamento fisso")],
...
async function onb2GeneraCore(dati,t){
  ...
  try{
    ...
    const plan=await wizGenDays(dati,t,null,(f,extra)=>{
      ...
      else if(f==="giorno"){g.fatti=extra.fatti;g.perc=8+Math.round(extra.fatti*10);
    ...
    if(plan&&plan.insicuro){
      let base=null;try{base=onb2PianoBase(dayTargetK()||t.kcal);}catch(e){}
      g.piano=base;
    ...
    g.riga=plan?tr("Piano pronto, spesa compresa."):tr("Il piano lo rifacciamo con calma da Piano: il diario intanto è già tuo.");
    g.righe=plan?onb2GenRighe(7,"fatto"):[];
  }catch(e){
    g.stato="errore";g.perc=100;g.righe=[];
    /* Il MOTIVO si scrive: «non è arrivato» senza un perché è il tipo
       di frase che costringe a indovinare — chiave, rete, quota? */
    const perche=(typeof aiReason==="function")?aiReason(e):"";
...
function onb2GenChiusa(){
  ...
  if(buono){
    ...
    if((S.onboard&&S.onboard.done)||g.applicaDaSolo){
      onb2PianoApplica(g.piano,spesa);
      ...
      try{S.genPronto={piano:JSON.parse(JSON.stringify(g.piano)),spesa:spesa,at:Date.now()};save();}catch(e){}
    }
...
function onb2PianoApplica(piano,spesa){
  S.customPlan=piano;PLAN=S.customPlan;S.permMeals={};
  S.customShop=spesa||null;S.shop={};S.week=freshWeek();
...
window.onb2Chiudi=async(modo)=>{
  ...
  if(gg.piano){
    onb2PianoApplica(gg.piano,(gg.stato==="fatto"&&gg.piano.spesa)?gg.piano.spesa:null);
