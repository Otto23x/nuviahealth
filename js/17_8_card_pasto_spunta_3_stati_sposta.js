...
function mealCard(pdi,mi){
  ...
  h+=`</select></div><div class="mtools" style="margin-top:8px">
    <button class="ibtn attrezzi-piu" title="${tr("Tutti gli attrezzi di questo pasto")}" aria-label="${tr("Tutti gli attrezzi di questo pasto")}" onclick="attrezziApri(${pdi},${mi})">${ic("attrezzi",18)}</button>`;
...
/* Il foglio degli attrezzi: apre `attrezziPasto` così com'è. Non
   ridisegna niente e non duplica un handler — se domani un attrezzo
   cambia, cambia in un posto solo e cambia in tutti e due i posti in
   cui la riga appare (il Punto e ogni pasto di Oggi). */
window.attrezziApri=(pdi,mi)=>{
  let nome="";
  try{
    const m=PLAN[pdi].meals[mi],st=S.week.days[pdi].meals[mi];
    nome=fascia((st&&st.movedAs)||m.n);
  }catch(e){nome=tr("Attrezzi");}
  sheetShow(nome,attrezziPasto(pdi,mi));};
