...
function guideCardHTML(){
  ...
  /* Riscontro del founder (25/08): questa card usciva MISTA in
     inglese — «Primi passi», «Fatto», «Vai», «Nascondi» e uno dei
     cinque passi erano rimasti fuori da tr(). È il difetto che i
     collaudi delle lingue non vedevano perché la card compare solo
     nei primi 14 giorni con un piano a metà: uno stato che nessun
     collaudo seminava. */
  return `<div class="card"><h2>${esc(tr("Primi passi"))} <span style="font-weight:600;color:var(--grigio);font-size:13px">${fatti}/5</span></h2>
  <div class="glist">
   ${GUIDE_STEPS.map(x=>{const ok=x.done();
     ...
     return `<button class="grow gstep ${ok?"on fatta":""}" onclick="guideLand('${x.page}','${sel}')">
       ...
       <div class="gc"><span class="glink">${ok?esc(tr("Fatto")):esc(tr("Vai"))+" ›"}</span></div></button>`;}).join("")}
  </div>
  <div style="text-align:right;margin-top:4px"><button class="glink" onclick="guideHide()">${esc(tr("Nascondi"))}</button></div></div>`;}
...
window.i18nDinamiche=function(){
  ...
  /* GLI ESERCIZI NON PASSANO PIÙ DI QUI (v13.92): le trenta voci si
     portano l'inglese addosso — «n» e «en», «mus» e «musEn» — e
     esNome()/esMuscoli() scelgono in base alla lingua. Un nome di
     esercizio non arriva mai a tr(), quindi non è una chiave: metterlo
     nel registro terrebbe in vita nel dizionario centoquattro voci che
     nessuno chiama più. Il gruppo («gambe», «petto») resta interno:
     serve a scegliere gli esercizi, non si scrive a schermo. */
  ...
  /* TAGLIO LINGUA (25/08): i contenuti seguono la lingua — quindi anche
  ...
  try{ALLERG_LIST.forEach(agg);}catch(e){}
  try{FARM_LIST.forEach(x=>agg(x.l));}catch(e){}
  try{INTEG_LIST.forEach(agg);}catch(e){}
...
function render(p){
  ...
  setTimeout(()=>{try{aiSpiaSincro();}catch(e){}},0);
  ...
  safeRender(p,fn);
  /* `filaIconeSegna` è sparita nella v13.86 con la fila di icone che
     segnava: sotto ogni pasto c'è un solo bottone «⋯», e niente esce
     più dal bordo. La chiamata è tolta invece di essere lasciata a
     girare a vuoto dietro il `typeof`. */
  setTimeout(()=>{try{faroApply(p);tipApply(p);a11yLega(p);}catch(e){}},90);}
