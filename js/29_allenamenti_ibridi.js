...
function abitualiAggiungi(sport,giorno,min){
  ...
  if(!s)return null;
  /* LA STESSA FAMIGLIA DEL BUG «Corsa · Domenica» (25/08): non
     isFinite(null), qui è `null>=0` — che in JS è VERO, perché null
     si converte a 0 nei confronti relazionali. Prima di questa
     guardia, chiamare abitualiAggiungi(sport,null,min) — «giorno non
     detto» — trasformava silenziosamente il giorno in domenica. La
     funzione non aveva mai avuto un chiamante che passasse null
     finché R1 (v13.90) non l'ha travasata dal percorso guidato: il
     collaudo l'ha preso al primo giro. */
  const g=(giorno!=null&&giorno>=0&&giorno<=6)?+giorno:null;
...
function abitualiValida(j){
  ...
  lista.slice(0,12).forEach(a=>{
    ...
    if(typeof a.giorno==="string"){
      ...
      if(i>=0)g=i;}
    /* LA VERA CAUSA del «Corsa · Domenica» (trovata dal collaudo,
       25/08): isFinite(null) è TRUE — null coerce a 0. L'AI
       rispettava il contratto e mandava null («giorno non detto»),
       e questo ramo lo trasformava in 0, cioè domenica. Il null va
       escluso PRIMA di chiedere se è un numero. */
    else if(a.giorno!=null&&isFinite(a.giorno)&&a.giorno>=0&&a.giorno<=6)g=+a.giorno;
    /* Il giorno non detto è OGGI (riscontro del founder, 25/08 sera:
       «ho solo detto corro per mezz'ora e lui ha messo domenica —
       oggi è martedì»). Chi racconta un allenamento senza data sta
       parlando di adesso, non di un giorno a caso della settimana.
       NB: GIORNI_NOMI comincia da DOMENICA, quindi l'indice è
       getDay() così com'è — la prima stesura lo spostava di uno,
       e il collaudo l'ha preso al primo giro. */
    if(g==null)g=new Date().getDay();
...
const ABIT_PROMPT='Questa persona racconta come si allena di solito: """{T}""". '+
  ...
  'Se non dice il giorno ESPLICITAMENTE, giorno=null: NON indovinare un giorno che non è stato detto. '+
  'Se non dice la durata lascia minuti a null. '+
