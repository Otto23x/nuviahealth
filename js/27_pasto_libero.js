...
/* ═══ LA DETTATURA SI VEDE (riscontro del founder, 25/08 sera) ═════
   «Dici che mi ascolti ma non riporti la frase che dico, non so che
   cosa sente il sistema, e nemmeno come disattivare il microfono.»
   Aveva ragione tre volte: il testo finiva in una textarea NASCOSTA
   (left:-9999px), l'unico segnale era un popup — ora spento — e il
   bottone del microfono non sapeva spegnersi.
   Adesso: quello che il sistema sente compare RIGA PER RIGA in un
   riquadro sotto il pasto, il microfono si spegne toccandolo di
   nuovo (o con «Ho finito»), e l'anello verde sta acceso solo
   mentre ascolta davvero. */
window.vocePasto=(btn)=>{
  ...
  const box=document.getElementById("fotoOut");
  /* secondo tocco sul microfono = stop e lettura */
  if(typeof VOCE!=="undefined"&&VOCE){
    try{voceIn("vocePastoTxt",btn);}catch(e){}
    const t=(ta.value||"").trim();
    const s=document.getElementById("vocePastoStop");if(s)s.remove();
    const l=document.getElementById("voceLive");if(l)l.remove();
    return vocePastoLeggi(t);}
  ta.value="";
  try{voceIn("vocePastoTxt",btn);}catch(e){
    return dlgAlert(tr("Il microfono non è partito. Puoi scrivere il pasto: è identico."));}
  if(box){
    box.style.display="block";
    box.innerHTML=`<div id="voceLive" class="hint" aria-live="polite">${esc(tr("Ti ascolto: parla pure, anche in disordine."))}</div>`;
    const vivo=document.getElementById("voceLive");
    ta.addEventListener("input",()=>{if(vivo)vivo.textContent=ta.value||"";});
    if(!document.getElementById("vocePastoStop")){
      const b=document.createElement("button");
      b.id="vocePastoStop";b.className="btn";b.type="button";
      b.textContent=tr("Ho finito");
      b.onclick=()=>{try{voceIn("vocePastoTxt",btn);}catch(e){}
        const t=(document.getElementById("vocePastoTxt")||{}).value||"";
        b.remove();const l=document.getElementById("voceLive");if(l)l.remove();
        vocePastoLeggi(t);};
      box.appendChild(b);}}};
