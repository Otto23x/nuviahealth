...
window.commensali=(pdi,mi,poi)=>{
  ...
  /* aperta dalla PENTOLA (poi=true) prosegue da sola verso la
     ricetta: il titolo lo dice, così le due icone — persone e
     pentola — non sembrano più «la stessa cosa» (riscontro 25/08:
  ...
  sheetShow(poi?tr("Chi mangia oggi — poi ti guido a cucinare"):tr("Chi mangia oggi"),
   `<div class="hint">${esc(tr("Vale per tutta la giornata: la spesa e le dosi si moltiplicano, la TUA porzione resta quella del piano."))}</div>
    ...
    <div class="mopts" id="cucAd">${[1,2,3,4,5,6].map(n=>
      ...
      <input type="number" id="cucAdN" min="1" max="60" inputmode="numeric" value="${c.a>6?c.a:""}"
        placeholder="${esc(tr("di più? scrivilo"))}" oninput="cucSetA(parseInt(this.value)||0,1)"
        style="width:110px;flex:none"></div>
    <label style="margin-top:12px">${esc(tr("Bambini"))}</label>
    <div class="mopts" id="cucBi">${[0,1,2,3,4].map(n=>
      `<button type="button" class="${n===c.b?"on":""}" onclick="cucSetB(${n})">${n}</button>`).join("")}
      <input type="number" id="cucBiN" min="0" max="60" inputmode="numeric" value="${c.b>4?c.b:""}"
        placeholder="${esc(tr("di più? scrivilo"))}" oninput="cucSetB(parseInt(this.value)||0,1)"
        style="width:110px;flex:none"></div>
...
let CUC_TMP={a:1,b:0};
/* ── ADULTI E BAMBINI INSIEME, E ANCHE IN TANTI (riscontro del
   founder, 25/08 sera): le due file sono sempre state indipendenti —
   4 adulti E 2 bambini si potevano scegliere — ma niente lo diceva, e
   sopra i 6 adulti (o 4 bambini) non c'era strada: per una tavolata
   da 12 il conto si fermava a 6. Il campo «di più? scrivilo» accetta
   il numero a mano; `daCampo` evita che scrivere nel campo azzeri la
   selezione dei bottoni quando il campo si svuota. */
window.cucSetA=(n,daCampo)=>{
  if(daCampo&&(!n||n<1))return;
  CUC_TMP.a=Math.max(1,Math.min(60,n||1));cucSegna("cucAd",CUC_TMP.a,1);};
window.cucSetB=(n,daCampo)=>{
  if(daCampo&&(n==null||isNaN(n)))return;
  CUC_TMP.b=Math.max(0,Math.min(60,n||0));cucSegna("cucBi",CUC_TMP.b,0);};
