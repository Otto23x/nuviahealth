...
async function wizGenDays(d,t,onStep,onFase){
  ...
  /* la frequenza cambia la regola: un integratore saltuario non
     entra nei target come uno quotidiano (riscontro 25/08) */
  ...
  const comune=' Persona: '+persona+
    ...
    /* PREVENIRE COSTA MENO CHE RIPARARE (v13.98): al modello si dice
       ...
       ricevere una settimana senza latticini: deve ricevere quelli
       senza lattosio, scritti per esteso. */
    (typeof alternativeRiga==="function"?alternativeRiga(d.intol||""):"")+
    /* i farmaci: non si discutono, si tiene conto di cosa vogliono a
       tavola — la vitamina K costante, il pompelmo fuori, il ferro
       lontano dal caffè */
    ((typeof farmaciRiga==="function")?farmaciRiga(d.farmaci||""):"")+
    /* quello che ha detto il medico viene PRIMA di qualunque
       preferenza: si dice al modello con queste parole */
    (d.medico?' INDICAZIONI DEL MEDICO, VINCOLANTI e sopra ogni altra preferenza: '+d.medico+'. Non contraddirle mai, nemmeno per rispettare i target.':'')+
    /* l'integrazione si propone solo a chi l'ha accettata */
    ((d.integrareOk==="no")
    ...
    /* ── LA VARIETÀ SCELTA CONTAVA DAVVERO ZERO (founder, 27/08) ──
       «Sembra non tenga conto del tutto della varietà selezionata: la
       varietà è media, ma ci sono un sacco di proteine e piatti
       diversi.» Aveva ragione, e la riga qui sotto era scritta a mano:
       «nessun piatto si ripete» è la definizione di varietà ALTA, e
       arrivava al modello qualunque cosa la persona avesse scelto.
       La regola vera esisteva già — `rigaVarieta` in 21_14, usata da
       «Genera nuovo piano» — e questa strada, quella del PRIMO piano
       di ogni persona, non la chiamava. Adesso la chiama: una regola
       sola, in un posto solo. */
    ((typeof rigaVarieta==="function")?rigaVarieta(d.varieta||"media")
  ...
  const esito=await chiediSettimana({
    ...
    regole:{giorni:DAYS,kcal:t.kcal,prot:t.prot,tollPct:5,
      ...
      allergeni:(d.allergeniLista&&d.allergeniLista.length)?d.allergeniLista
                :((typeof allergeniElenco==="function")?allergeniElenco(d.allergie||""):[]),
  ...
  if(!esito.plan){
    await dlgAlert(tr("Le ricette non sono arrivate{e}.",{e:(esito.err?" ("+aiReason(esito.err)+")":"")})+
      "\n\n"+tr("Le tue risposte sono al sicuro: si riprova da Ricette, quando vuoi."));
    return null;}
  /* ── LA SICUREZZA SI RIPARA, NON BLOCCA (v13.98) ────────────────
     `chiediSettimana` ha già sostituito ogni divieto rimasto con un
     alimento sicuro per questa persona: qui non arriva più un piano
     insicuro da rifiutare, arriva un piano SISTEMATO con l'elenco di
     cosa è stato cambiato. `insicuro` resta solo come rete: se un
     giorno la riparazione non bastasse, chi chiama può ancora
     saperlo — ma non è più la strada normale. */
  const insicuri=(esito.problemi||[]).filter(x=>!x.sicuro);
  const plan=esito.plan;
  /* la lista si normalizza QUI, alla sorgente: da questo campo passa
     sia il percorso guidato sia il wizard, e normalizzare in fondo a
     ogni strada e' come non normalizzare - una strada la si dimentica
     sempre (i «[object Object]» del 27/08 sono usciti proprio da una
     strada dimenticata). */
  plan.spesa=((typeof normSpesaAI==="function")?normSpesaAI(esito.spesa):esito.spesa)||null;
  plan.problemi=esito.problemi||[];
  plan.adattati=esito.adattati||[];
...
async function wizGenerate(){
  ...
  try{
    ...
    if(plan.adattati&&plan.adattati.length)
      await dlgAlert(adattatiTesto(plan.adattati));
    box.textContent="";
...
function rateNote(){
  ...
  return trh("il ritmo richiesto ({v1} a settimana) supera il tetto di sicurezza",{v1:((typeof pesoTxt==="function")?pesoTxt(ratePerWeek(),1):ratePerWeek()+" kg")})+
    (su?" del surplus":" del 30% del fabbisogno")+trh(": le ricette lavorano a ~{v1} a settimana",{v1:((typeof pesoTxt==="function")?pesoTxt(rateEffective(),1):rateEffective()+" kg")});}
...
${trh("      ⏳ {b1}: questi stati cambiano le calorie del giorno.",{b1:"<b>"+tr("Spunta solo ciò che è vero adesso")+"</b>"})}
  ...
  return `<div class="hint" style="background:var(--zaffbg);padding:8px 12px;border-radius:12px;margin-top:8px">
    ⏳ <b>${tr("Stati attivi adesso")}</b> ${tr("— stanno cambiando le calorie di oggi e le porzioni delle ricette.")}
...
window.famPianoSet=(v)=>{S.famRicette=!!v;save();
  ...
  toast(v?tr("Cucinerò per tutti — dalla prossima generazione delle ricette"):tr("Piatti pensati solo per te — dalla prossima generazione delle ricette"));};
window.famAdd=()=>{
...
window.weekOutSet=async()=>{
  ...
  if(!aiOn())return dlgAlert(tr("Selezioni salvate.\n\nSenza chiave AI le ricette non possono riadattarsi da sole: rigenera la spesa a mano dalla pagina Spesa."));
...
window.outTypeSet=async(v,perSettimana)=>{
  ...
  if(aiOn()){
    if(await dlgConfirm(v==="porto"
      ...
      {ok:tr("Adatta ricette e spesa"),ko:tr("Solo il tipo")}))
...
function outTypeHTML(pre,perSettimana){
  ...
  <div class="hint">${c2==="porto"
    ...
    :tr("Mensa o ristorante: le ricette dicono <b>come comporre il piatto</b>, e quegli ingredienti <b>non vanno in spesa</b>.")}</div>`;}
...
function famCardHTML(pre){
  ...
  ${(S.family||[]).length?`<label class="ckline" style="margin-top:12px"><input type="checkbox" id="famRicetteCk" ${S.famRicette?"checked":""}
    onchange="famPianoSet(this.checked)"> ${tr("Nelle ricette, scegli piatti che posso cucinare per tutti")}</label>`:""}
...
window.wizConfirm=async ()=>{
  ...
  if(!await dlgConfirm(tr("Confermo queste ricette come base settimanale? La settimana in corso (spunte, extra, allenamenti) viene azzerata.")+(WIZ.editOnly?"":" "+tr("Consulta comunque un nutrizionista prima di seguirlo."))))return;
  snapSave("prima di: ricette modificate");
  ...
  dlgAlert(tr("Ricette confermate! Vai in Spesa per generare la lista della spesa dalle nuove ricette."));
  show("ricette");};
function renderSetup(){const el=document.getElementById("pg-setup");let h="";
  const disc=`<div class="card nota grave">${hint2(tr("<b> Avvertenza importante:</b> queste ricette sono generate da un'intelligenza artificiale su dati auto-dichiarati."),tr("NON sostituisce un professionista: prima di seguirlo, fallo validare da un nutrizionista o un medico, soprattutto in presenza di patologie, intolleranze o gravidanza."))}</div>`;
  if(WIZ.step===1){h=disc+`<div class="card"><h2>${tr("Passo 1 di 3 — Chi sei")}</h2>
    ...
    <div class="hint">${t.lbm?trh("Massa magra stimata: {v1} (dalla % di grasso). ",{v1:((typeof pesoTxt==="function")?pesoTxt(t.lbm,1):t.lbm+" kg")}):""}${trh("Con l'obiettivo selezionato le ricette punteranno a {v1} kcal e {v2} di proteine al giorno (ricalcolati quando premi Genera).",{v1:'<b id="wzK">'+t.kcal+"</b>",v2:"<b>"+t.prot+"g</b>"})}</div>
    ...
    <label style="margin-top:8px"><input type="checkbox" id="wzOk" style="width:auto"> ${tr("Ho letto l'avvertenza: farò validare le ricette da un nutrizionista/medico")}</label>
    ...
    h=(WIZ.editOnly?`<button class="btn ghost small" style="margin-bottom:12px" onclick="show('ricette')">${tr("‹ Torna alle ricette (senza salvare)")}</button>`:disc)+`<div class="card"><h2>${WIZ.editOnly?"Modifica il piano":" Il tuo piano — controlla e modifica"}</h2>
    ...
    <button class="btn" onclick="wizConfirm()">${tr("Conferma ricette")}</button>
    <button class="btn ghost" onclick="show('io')">${tr("Annulla")}</button></div>
    <div class="hint"> ${tr("Ricorda: fai validare le ricette da un nutrizionista prima di seguirle.")}</div></div>`;}
...
<table class="gtable">
<tr><td colspan="2"><b>Percorso guidato</b> ${trh("— otto passi guidati: privacy, chiave AI, chi sei, obiettivo, come mangi, backup, ricette. La {b1}, così tutti i passi successivi possono già usare l'AI.",{b1:"<b>chiave Gemini si aggancia subito al passo 3</b>"})}</td></tr>
...
<tr><td colspan="2"><b>${tr("Pasti fuori casa")}</b> — non più solo "mensa": segni i giorni in cui mangi fuori (mensa, trattoria, bar, menù fisso) e per ognuno se è <b>pranzo, cena o entrambi</b> ${tr("— utile a chi lavora in viaggio. Il totale settimanale si conta da sé, e per quei pasti le ricette descrivono come comporre il piatto invece di inventarne uno.")}</td></tr>
<tr><td colspan="2"><b>${tr("Varietà a tre livelli")}</b> ${tr("— scegli tu se le ricette devono girare su pochi ingredienti che tornano (spesa corta, poco da cucinare), stare in mezzo, o proporre piatti sempre diversi. L'AI costruisce la settimana di conseguenza.")}</td></tr>
<tr><td colspan="2"><b>${tr("Le ricette seguono il tuo peso")}</b> ${trh("— quando cali (o sali) di oltre {v1} rispetto a quando le ricette sono nate, l'app te lo dice, mostra come è cambiato il fabbisogno e propone di ritarare le grammature. Mai in automatico: decidi tu.",{v1:(typeof pesoTxt==="function")?pesoTxt(3,0):"3 kg"})}</td></tr>
<tr><td colspan="2"><b>Mensa davvero generica</b> ${tr("— per i pasti in mensa le ricette non inventano un piatto preciso, ma descrivono come comporre il vassoio (fonte proteica, contorno, porzione di carboidrati). Quei pasti non finiscono mai nella lista della spesa.")}</td></tr>
...
<tr><td colspan="2"><b>Stima risultati</b> ${trh("— media delle ricette vs fabbisogno + allenamenti = kg/settimana e data stimata dell'obiettivo; se l'obiettivo non torna, propone la {b1} (i piatti restano gli stessi) con spesa e stima rigenerate.",{b1:"<b>"+tr("ritaratura automatica delle grammature")+"</b>"})}</td></tr>
<tr><td colspan="2"><b>Alternative stagionali</b> ${tr("— a pranzi e cene si aggiunge un'opzione di stagione etichettata")} ${ic("primavera",14)} Primaverile · ${ic("estate",14)} Estivo · ${ic("autunno",14)} Autunnale · ${ic("inverno",14)} ${tr("Invernale, a parità di kcal e proteine; le ricette di base restano generiche e rigenerando spariscono le stagioni vecchie.")}</td></tr>
...
<tr><td colspan="2"><b>${tr("Annulla")}</b> ${trh("— ogni azione che cambia i dati si può disfare: accanto al messaggio in fondo allo schermo compare {b1}, per otto secondi (quindici quando l'azione è pesante, come svuotare la dispensa o riscrivere le ricette). Si annulla l'ultima azione, una sola volta: è un ripensamento, non una cronologia. I salvataggi automatici dell'avvio non sono annullabili, perché non li hai fatti tu.",{b1:"<b>"+tr("Annulla")+"</b>"})}</td></tr>
...
<tr><td colspan="2"><b>${tr("Alternative della spesa che aggiornano le ricette")}</b> ${trh("— l'icona AI su un prodotto propone tre sostituti {b}: quello che scegli entra in lista e, se accetti, riscrive le ricette che lo usavano adattando le grammature per restare vicino a kcal e proteine originali.",{b:"<b>selezionabili</b>"})}</td></tr>
...
<tr><td colspan="2"><b>${tr("Lista della spesa")}</b> — derivata dal ${tr("<b>tue</b> ricette e <b>rigenerata da sola</b> quando le ricette cambiano, con AI o senza, categorie da supermercato, spunte persistenti, link al tuo negozio, invio WhatsApp.")}</td></tr>
...
NUVIA_IT.corpo=()=>`<div class="card nvhero">
  <img class="nvfloat" alt="Nuvia" src="assets/marchio-esteso.svg" style="width:min(60%,220px);height:auto" onerror="this.src=window.NUVIA_LOCKUP||''">
...
<div class="gsec">${tr("2 bis · Il target delle ricette (e perché non è il fabbisogno)")}</div>
<p class="gp">${trh("Le calorie delle ricette non coincidono col fabbisogno: sono {b}. Il deficit si fissa in due modi, a scelta in {b2}: come {b4} del fabbisogno (20% significa mangiare l'80% di quello che consumi) oppure partendo dai {b3} desiderati, sapendo che un chilo di grasso vale circa 7.700 kcal. Il tetto è il 30%.",{b:"<b>"+tr("fabbisogno meno il deficit")+"</b>",b2:"<b>Regole</b>",b4:"<b>percentuale</b>",b3:"<b>chili a settimana</b>"})}
...
<br><br>Quello che <b>non cambia</b> sono le grammature delle ricette: quelle restano tue, le decidono i tuoi numeri. Se le ricette moltiplicassero le porzioni per il numero di persone sarebbe sbagliato di tre volte restando perfettamente sensato a leggerlo — per questo la famiglia entra nella scelta dei piatti e mai nelle quantità.
...
<tr><td colspan="2"><b>Oggi</b> ${trh("— bilancio del giorno (anello con il {b1} in grande e {b2} = le kcal previste dalle ricette per quel giorno), acqua, sonno/relax/umore, pasti ed extra da spuntare. Le frecce ‹ › in alto cambiano giorno.",{b1:"<b>deficit</b>",b2:"<b>Pianificato</b>"})}</td></tr>
<tr><td colspan="2"><b>${tr("Ricette")}</b> ${tr("— tutta la settimana con i sei comandi: gestisci · alternativa stagionale · ripeti il percorso guidato · genera nuove ricette · importa da foto · stima risultati; più opzioni, orari e alternative per ogni pasto.")}</td></tr>
<tr><td colspan="2">${trh("<b>Spesa</b> — lista che si costruisce e si aggiorna da sola dalle ricette, ricerca sul sito del tuo supermercato, invio WhatsApp. L'icona AI su un prodotto propone {b1}: quella che scegli sostituisce il prodotto in lista e, se vuoi, <b>riscrive le ricette</b> che lo usavano. Dallo {b3} arrivano prodotti, quantità e {b2}: da lì Nuvia calcola quanto è costata la spesa, quanto costa in media un pasto e dove vanno i soldi.",{b1:"<b>tre alternative da scegliere</b>",b3:"<b>scontrino</b>",b2:"<b>prezzi</b>"})}</td></tr>
...
<tr><td colspan="2"><b>Strumenti</b> — quello che serve in un momento preciso: «Ho una voglia…» (l'app cerca un'alternativa), «Sto per cedere» (non si dice di no: si dice metà, e adesso), la dispensa, il codice ricette.</td></tr>
...
<tr><td colspan="2"><b>Abbonamento</b> e <b>Gli abbonamenti</b> — cosa è attivo adesso e cosa aggiunge ciascuno. Free resta gratis.</td></tr>
<tr><td colspan="2"><b>Guida</b> e <b>Nuvia</b> — questa guida, e la mappa di tutte le funzioni una per una.</td></tr>
<tr><td colspan="2"><b>Percorso guidato</b> ${tr("(in Ricette) — lo stesso percorso guidato, con tutti i campi già compilati (dati, intolleranze, gusti, obiettivi) da cui l'AI propone una settimana di ricette, tutta modificabile prima della conferma. «Modifica le ricette» per ritocchi a mano. Sempre da validare con un nutrizionista.")}</td></tr></table></div></details></div>`;
...
<tr><td colspan="2"><b>${tr("Le ricette non si muovono")}</b>: ribilanciamenti, foto e correzioni cambiano solo la giornata in Oggi. Il previsto del giorno resta quello delle ricette, così il confronto "previsto vs mangiato" ha sempre senso.</td></tr>
...
<tr><td colspan="2">${tr("Verde = pasto normale delle ricette.")}</td></tr>
...
<tr><td colspan="2"><b>Caratteristiche alimentari</b> ${tr("(sezione Regole): impostazione di riferimento (mediterranea, low carb, digiuno intermittente, vegetariana…), pasti al giorno e quali salti, pasti liberi, pasti fuori casa e mensa, tempo per cucinare, budget, intolleranze, vincoli religiosi, condizioni da tenere presenti. Da qui l'AI può anche")} <b>${tr("scrivere le ricette complete")}</b>.</td></tr>
<tr><td>${ic(seasonNow(),18)}</td><td>${tr("<b>Opzioni stagionali</b>: tra le opzioni dei pasti principali, l'etichetta")}${ic("primavera",14)} Primaverile · ${ic("estate",14)} Estivo · ${ic("autunno",14)} Autunnale · ${ic("inverno",14)} ${tr("Invernale indica l'alternativa di stagione creata con «Alternativa stagionale» in Ricette. Le ricette di base restano intatte; rigenerando, le stagioni vecchie vengono sostituite.")}</td></tr>
<tr><td colspan="2"><b>Stima risultati</b> ${trh("(in Ricette): bilancio delle ricette → kg a settimana e data stimata dell'obiettivo; se l'obiettivo non sembra raggiungibile, ti propone di {b} automaticamente (con spesa e stima rigenerate) o di mantenere tutto com'è.",{b:"<b>"+tr("ritarare le grammature")+"</b>"})}</td></tr>
...
<tr><td colspan="2"><b>${tr("Prodotti in lista")}</b> ${tr("(in Spesa): arrivano tutti dalle ricette, compresi gli ingredienti dei pasti che modifichi o aggiungi tu — la lista si riallinea da sola. Per una cosa fuori dal previsto usa il ＋ accanto alla categoria.")}</td></tr>
...
<tr><td>${ic("ricette",18)}</td><td>Ricette — la settimana.</td></tr>
<tr><td>${ic("spesa",18)}</td><td>Spesa — la lista che nasce dalle ricette.</td></tr>
...
<tr><td>${ic("tools",18)}</td><td>Strumenti — le voglie, la dispensa, il codice ricette.</td></tr>
...
<tr><td colspan="2"><b>Il pallino della qualità</b> (su ogni pasto delle ricette): un voto da 0 a 100 sulla qualità degli alimenti, non sulle calorie. 0-20 ultraprocessato · 21-40 scarso · 41-60 nella media · 61-80 buono, cibo vero · 81-100 ottimo. Le ricette puntano a restare sopra 75, e un pasto libero basso non è un errore: è un pasto libero.</td></tr>
...
<div class="card"><div class="hint"> ${trh("Le stime di calorie, proteine e consumi sono {b}: l'app è un diario, non un medico. Le ricette vanno validate col tuo professionista.",{b:"<b>indicative</b>"})}</div>
</div>`;
...
<div class="card"><div class="hint"> Calorie, protein and energy figures are <b>indicative</b>: this is a diary, not a doctor. Have your recipes validated by your own professional.</div>
</div>`;
...
The <b>Recipes</b> is a fixed model: the sum of the meals planned for that day. It never changes on its own — if you rebalance, photograph a dish or correct something with , what changes is the <b>day</b> in Today, not the recipes.\nWhat you <b>ate</b> is the sum of the meals you ticked ✓ plus any extras you haven't crossed out. Meals marked ✗ and extras marked ✗ don't count.
...
<br><br>What does <b>not</b> change is the amounts in your recipes: those stay yours, decided by your own numbers. If the recipes multiplied the portions by the number of people it would be wrong by three times while reading perfectly sensibly — which is why the family enters the choice of dishes and never the quantities.
...
<tr><td colspan="2"><b>Today</b> — the day's balance: a ring with the <b>deficit</b> in large type and <b>Planned</b> (the kcal your recipes set for that day), water, sleep/relaxation/mood, meals and extras to tick off. The ‹ › arrows at the top move between days.</td></tr>
<tr><td colspan="2"><b>Recipes</b> — the whole week with its six commands: manage · seasonal alternative · repeat the guided setup · generate new recipes · import from photos · estimate results; plus options, times and alternatives for every meal.</td></tr>
<tr><td colspan="2"><b>Shopping</b> — a list that builds and updates itself from your recipes, searches your supermarket's own site, and sends by WhatsApp. The AI icon on a product offers <b>three alternatives to choose from</b>: the one you pick replaces the product on the list and, if you want, <b>rewrites the recipes</b> that used it. From your <b>receipt</b> come products, quantities and <b>prices</b>: from there Nuvia works out what the shop cost, what an average meal costs, and where the money goes.</td></tr>
...
<tr><td colspan="2"><b>Recipes code</b> (under Rules → The recipes, or in the last step of the welcome) — loads ready-made recipes. <b>00000000</b> is the standard diet Nuvia was born with: Mediterranean, five meals, canteen on Tuesdays and Thursdays.</td></tr>
<tr><td colspan="2"><b>Rules</b> — the configuration, in <b>three tabs</b>: <b>Your choices</b> (what you entered during the guided setup: goal, dietary details, protocols — you change it here and it counts everywhere), <b>AI rules</b> (how the AI should reason, "What the AI knows about you" with the exact text sent with every request, WHO guidelines), and <b>Formulas and calculations</b>: energy needs, the workout formula, rebalancing and catch-up thresholds, protein goal, dietary details and recipes management.</td></tr>
...
<tr><td colspan="2"><b>Guided setup</b> (under Recipes) — the same guided path, with every field already filled in (your data, intolerances, tastes, goals), from which the AI generates a week made for you, editable before you confirm it. "Edit the recipes" for changes by hand. Always to be validated with a nutritionist.</td></tr><tr><td colspan="2"><b>The point</b> — the page that opens first: how things are going right now and, if you have just arrived, the first steps to take. Everything else opens from here.</td></tr>
...
<tr><td colspan="2"><b>Tools</b> — what you need at one precise moment: "I have a craving…" (the app looks for an alternative), "I'm about to give in" (nobody says no: it says half, and now), the pantry, the recipes code.</td></tr>
...
<tr><td colspan="2"><b>Subscription</b> and <b>Plans</b> — what is active now and what each recipes does. Free stays free.</td></tr>
...
<tr><td colspan="2"><b>The recipes doesn't move</b>: rebalancing, photos and corrections change the day in Today only. The day's planned figure stays the one from the recipes, so the "planned vs eaten" comparison always means something.</td></tr>
...
<tr><td colspan="2">Green = an ordinary meal from the recipes.</td></tr>
...
<tr><td colspan="2"><b>Dietary details</b> (Rules section): your reference style (Mediterranean, low carb, intermittent fasting, vegetarian…), meals per day and which ones you skip, free meals, meals away from home and canteen, time to cook, budget, intolerances, religious constraints, conditions to keep in mind. From here the AI can also <b>generate a complete recipes</b>.</td></tr>
<tr><td>${ic(seasonNow(),18)}</td><td><b>Seasonal options</b>: among the options for main meals, the label ${ic("primavera",14)} Spring · ${ic("estate",14)} Summer · ${ic("autunno",14)} Autumn · ${ic("inverno",14)} Winter marks the seasonal alternative created with "Seasonal alternative" in Recipes. Your base recipes stay intact; regenerating replaces the old seasons.</td></tr>
<tr><td colspan="2"><b>Estimate results</b> (in Recipes): the recipes' balance → kilos per week and the estimated date for your goal; if the goal doesn't look reachable, it offers to <b>re-tune the amounts</b> automatically (rebuilding shopping list and estimate) or to leave everything as it is.</td></tr>
...
<tr><td colspan="2"><b>Products on the list</b> (in Shopping): they all come from the recipes, including the ingredients of meals you change or add yourself — the list realigns itself. For something outside the recipes, use the ＋ next to the category.</td></tr>
...
<tr><td>${ic("ricette",18)}</td><td>Recipes — the week.</td></tr>
<tr><td>${ic("spesa",18)}</td><td>Shopping — the list that comes from the recipes.</td></tr>
...
<tr><td>${ic("tools",18)}</td><td>Tools — cravings, pantry, recipes code.</td></tr>
...
<tr><td colspan="2"><b>The quality dot</b> (on every meal in the recipes): a score from 0 to 100 on the quality of the food, not on the calories. 0-20 ultra-processed · 21-40 poor · 41-60 average · 61-80 good, real food · 81-100 excellent. The recipes aim to stay above 75, and a low free meal is not a mistake: it is a free meal.</td></tr>
...
function renderSistema(){const el=document.getElementById("pg-sistema");const p=S.profile;
 ...
 tr("Serve per: foto dei piatti e del frigo, lettura dei menù, stime di sfori e sport, alternative, ribilanciamento, sostituzioni nella spesa e report settimanali. Senza chiave l'app funziona lo stesso, ma tutto va fatto a mano.")):""}</div>`;
  ...
  {const st=S.streak||{},ws=(S.profile.weights||[]).length,pr=(S.periods||[]).length;
   ...
   ${hint2(tr("Azzera <b>solo</b> quello che scegli, tenendo intatto tutto il resto:"),tr("Profilo, ricette, regole e caratteristiche non vengono toccati. Prima di procedere viene creata un'istantanea, quindi si può tornare indietro dal <b>Ripristino di emergenza</b>."))}
...
function derivaFrase(giu){
  ...
  if(/mantenimento/.test(g))
    return giu?tr("Con le ricette di prima mangeresti sopra il fabbisogno di adesso.")
              :tr("Con le ricette di prima mangeresti sotto il fabbisogno di adesso.");
  if(/massa|aument/.test(g))
    return giu?tr("Con le ricette di prima il surplus è aumentato.")
              :tr("Con le ricette di prima il surplus si è ridotto e la crescita rallenta.");
  return giu?tr("Con le ricette di prima il deficit si è ridotto e il calo rallenta.")
            :tr("Con le ricette di prima il deficit è aumentato.");}
window.checkPlanAge=async()=>{
  ...
  const giu=d>0;
  const msg=" "+(giu?tr("Hai perso {d} da quando sono state scritte queste ricette ({a} → {b}).",{d:((typeof pesoTxt==="function")?pesoTxt(d,1):d+" kg"),a:((typeof pesoNum==="function")?pesoNum(S.ricetteW,1):S.ricetteW),b:((typeof pesoTxt==="function")?pesoTxt(S.profile.w,1):S.profile.w+" kg")})
         :tr("Sei salito di {d} da quando sono state scritte queste ricette ({a} → {b}).",{d:((typeof pesoTxt==="function")?pesoTxt(Math.abs(d),1):Math.abs(d)+" kg"),a:((typeof pesoNum==="function")?pesoNum(S.ricetteW,1):S.ricetteW),b:((typeof pesoTxt==="function")?pesoTxt(S.profile.w,1):S.profile.w+" kg")}))+
    ...
    "\n\nPosso ritarare le grammature delle ricette attuali sui nuovi numeri, tenendo gli stessi piatti. Oppure lasci tutto com'è: i calcoli del diario usano comunque il peso di oggi.";
  if(!await dlgConfirm(msg,{ok:tr("Ritara le ricette"),ko:tr("Lascia com'è")}))return;
  if(!aiOn())return dlgAlert(tr("Per ritarare serve la chiave AI. Puoi comunque rigenerare le ricette a mano da Ricette."));
...
/* ═══ LE MISURE SI SCRIVONO IN UN POSTO SOLO ═════════════════════════
   Founder, 27/08: «Le cose che ti ho detto di aggiungere devono poter
   essere modificate nelle regole come tutto il resto, e non essere
   ridondanti in altre parti dell'app per non fare confusione.»
   La scheda «Dettagli» del percorso guidato chiede le stesse misure di
   «Misure dello studio» — massa grassa, massa magra, circonferenze,
   pliche — e quindi NON può avere un magazzino suo: scrive qui, nella
   stessa visita, con le stesse regole. Chi compila nel percorso ritrova
   tutto in Io → Misure, e lo corregge là. Una fonte sola.
   `misureRegistra` è quel posto: prende un record, lo fonde con la
   visita di oggi se c'è, tiene in ordine e aggiorna il profilo. */
window.misureRegistra=(rec)=>{
  rec=Object.assign({d:iso(new Date()),pliche:{},circ:{}},rec||{});
  const pieno=rec.fat||rec.mus||rec.acqua||rec.bmr||rec.pa||rec.note
    ||Object.keys(rec.pliche||{}).length||Object.keys(rec.circ||{}).length;
  if(!pieno)return false;
  const v=visite();
  const oggi=v.findIndex(x=>x.d===rec.d);
  if(oggi>=0){
    /* fondere e non sovrascrivere: pliche e circonferenze scritte in due
       momenti dello stesso giorno devono sommarsi, non cancellarsi */
    const p=Object.assign({},v[oggi].pliche||{},rec.pliche||{});
    const c=Object.assign({},v[oggi].circ||{},rec.circ||{});
    v[oggi]=Object.assign({},v[oggi],rec,{pliche:p,circ:c});
  }else v.push(rec);
  v.sort((a,b)=>a.d<b.d?-1:1);
  /* la composizione corporea è anche profilo: è lo stesso dato, e il
     piano la legge da lì */
...
function studioCardHTML(){
  ...
  ${hint2(tr("Le misure che prende un professionista — nutrizionista, centro dimagrimento, palestra — e le sue note."),tr("Entrano nell'analisi insieme alle ricette, agli allenamenti e a come stai."))}
