...
function intolChecksHTML(pre,D){const cur=String(D.intol||"").toLowerCase();
  let h=`<div class="ckgrid">`+INTOL_LIST.map((x,i)=>`<label class="ck"><input type="checkbox" id="${pre}I${i}" ${cur.indexOf(x.split(" ")[0])>-1?"checked":""}${x==="cibi acidi"?` onchange="var w=document.getElementById('${pre}AcidW');if(w)w.style.display=this.checked?'block':'none'"`:""}> ${(y=>y[0].toUpperCase()+y.slice(1))(tr(x))}</label>`).join("");
  h+=`</div>`;
...
function riparaDesc(desc,vietati){
  ...
  for(let giro=0;giro<8;giro++){
    ...
    if(!v)break;
    const cand=(SOSTITUTI[v]||[]).concat(RIPIEGO_FAMIGLIA);
...
window.genPlanAI=async()=>{
  ...
  try{
    ...
    if(adattati.length){if(box)box.textContent="";
      await dlgAlert(adattatiTesto(adattati));
      genPassi(box,7,2);}
